...
Если вы хотите сделать что-то с данными в Hoodie, то именно эта часть API вам поможет.

**По прочтению данного руководства, вы узнаете как:**

- добавлять объекты
- искать объекты
- обновлять объекты
- удалять объекты
- реагировать на изменения объектов

## Введение

Одной из главных функциональностей любого web-приложения является сохранение и получение данных, и **hoodie.store** это ваш инструментарий для любых операций с данными. Hoodie предоставляет каждому пользователю его собственное хранилище данных, приватное по-умолчанию и автоматически синхронизирующееся между клиентскими приложениями пользователя и сервером. Изначально, ваше приложение не имеет средств для того, чтобы сделать данные глобальными или расшарить их между пользователями, однако, эта функциональность предоставляется с помощью [плагинов](/ru/plugins/). Но вы сразу можете быть уверены в том, что данные синхронизированы между сессиями пользователя, а также доступны для редактирования оффлайн (с последующей синхронизацией).

#### Что вы должны знать, перед тем как продолжить

If you haven't read up on how Hoodie works under the hood (sorry) and does all your data syncing, then [now would be a good time](/ru/hoodieverse/how-hoodie-works.html). It's just a high-level guide that helps you understand the basic architecture and capabilities of Hoodie.

It is also important to understand the [concept of type in Hoodie](/ru/tutorials/#understanding-type), since it informs nearly all operations with data.
...
Allows you to attach listeners to the store, so your interface can react to changing data. Highly relevant when building properly decoupled frontends, [as explained here](/ru/tutorials/#updating-the-view), which will fully unlock the offline and cross-device syncing benefits of Hoodie.

**Important:** if you haven't yet, please read [the general guide to Hoodie events and their naming syntax](/ru/techdocs/api/client/hoodie.html#on) to better understand how events are handled. This page also describes the [one()](/ru/techdocs/api/client/hoodie.html#one) and [off()](/ru/techdocs/api/client/hoodie.html#off) methods for attaching a one-time listener, and for detaching all listeners.
...
**Important:** if you haven't yet, please read [the general guide to Hoodie events and their naming syntax](/ru/techdocs/api/client/hoodie.html#on) to better understand how events are handled.
...
Wow, excellent, you've reached the end of the core docs! The next logical step would be to [find out about Hoodie plugins](/ru/plugins/). Have fun!
...
If you find this guide in error or out of date, you could also <a href="https://github.com/hoodiehq/documentation/issues" target="_blank">open an issue</a> or submit a pull request with your corrections to <a href="https://github.com/hoodiehq/documentation/blob/gh-pages/ru/techdocs/api/client/hoodie.store.md" target="_blank">this file</a>.
